...
model Patient_PKPG_1C_Lithium
  ...
  Sources.PeriodicDose_Enteral dose(firstAdminTime(displayUnit = "s") = 60, adminMass = 0.0015, adminDuration(displayUnit = "s") = 60, F = 0.8, adminPeriod = 86400, doseCount = 7) annotation(
    Placement(transformation(origin = {-12, 22}, extent = {{-10, -10}, {10, 10}})));
  Pharmacokinetic.NoPerfusedTissueCompartment central(V = 0.225) annotation(
    Placement(transformation(origin = {-12, -8}, extent = {{-10, -10}, {10, 10}})));
  Pharmacokinetic.ClearanceDrivenElimination elim(CL = 2.9166666666666666e-6, useClInput = true) annotation(
  ...
  Sources.ClearanceConst elimClearance(Cl = 2.9166666666666666e-6) annotation(
