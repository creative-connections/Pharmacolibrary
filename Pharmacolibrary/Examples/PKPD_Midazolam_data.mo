...
model PKPD_Midazolam_data
  extends Examples.PK_2C_IVMidazolam(elim(CL = 3.3333333333333335e-5));
  ...
  parameter Modelica.Units.SI.Time dataTimeScale = 7.7 "Translation from rat to man" annotation(
    Evaluate = false);
  Optimization.Criteria.Signals.IntegratedSquaredDeviation integratedSquaredDeviation annotation(
    Placement(transformation(extent = {{32, 62}, {52, 82}})));
equation
  ...
  connect(CTT_MidazolamEffect_Epileptic.y[1], integratedSquaredDeviation.u1) annotation(
    Line(points = {{-79, 40}, {22, 40}, {22, 78}, {30, 78}}, color = {0, 0, 127}));
  connect(PD_Midazolam.effect, integratedSquaredDeviation.u2) annotation(
    Line(points = {{60, -46}, {60, 52}, {30, 52}, {30, 66}}, color = {0, 0, 127}));
  annotation(
